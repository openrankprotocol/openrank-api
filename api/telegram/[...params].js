...
module.exports = async (req, res) => {
  ...
  if (!params) {
    ...
    const match = url.match(/\/telegram\/([^?]+)/) || url.match(/^\/([^?]+)/);
    if (match && match[1]) {
  ...
  if (!Array.isArray(params)) {
    // If it's a string like "decentraliseddotco/seed", split it
  ...
  // Load the dataset
  const data = await loadDatasetAsync("telegram", fileName);
  ...
  if (endpoint === "channel_id") {
    return sendResponse(res, 200, { channel_id: data.channel_id });
  }
